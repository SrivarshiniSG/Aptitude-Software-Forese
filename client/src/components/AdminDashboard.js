...
export default function AdminDashboard() {
  ...
  const updateDepartments = async () => {
    try {
      ...
      await api.post('/api/admin/departments/update', {
    ...
    try {
      ...
      if (response.data.length > 0) {
        setAvailableDepartments(departments.map(dept => ({
          ...dept,
          isActive: response.data.find(d => d.id === dept.id)?.isActive ?? true
        })));
      } else {
        setAvailableDepartments(departments.map(dept => ({
          ...dept,
  ...
  return (
    <div className="container">
      ...
      <div className="departments-section">
        ...
        <p className="department-info">Select the departments that should be available in the login page:</p>
